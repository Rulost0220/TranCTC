...
# TRCTC_PARAMETERS = {
# 	"ipds_count": IPDS_COUNT,
# 	"legit_dataset_path": REFERENCE_DATA_DIRPATH + "http_intervals.csv",
...
def collect_http_intervals(test_dir, save_dir):
	ipd_lists = []
	...
	os.makedirs(save_dir, exist_ok=True)
	http_save_path = os.path.join(save_dir, 'http_intervals.csv')
	merged = []
	for ipd_list in ipd_lists:
		merged.extend(ipd_list)
	pd.DataFrame({'IPDs': merged}).to_csv(http_save_path, index=False)
	print(f"Saved http intervals to {http_save_path}")
...
def update_paths_and_count(reference_dir, save_dir, ipds_count):
	...
	TRCTC_PARAMETERS["legit_dataset_path"] = os.path.join(REFERENCE_DATA_DIRPATH, "http_intervals.csv")
	TRCTC_PARAMETERS["save_generated_ipds_path"] = os.path.join(IPDS_SAVE_DIRPATH, "TRCTC.csv")
	Jitterbug_PARAMETERS["legit_dataset_path"] = os.path.join(REFERENCE_DATA_DIRPATH, "http_intervals.csv")
...
if __name__ == "__main__":
	...
	if args.collect_reference:
		collect_http_intervals(args.test_dir, args.reference_dir)
