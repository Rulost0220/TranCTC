...
def scores_calculate(model_type, scores_mode, model, criterion, csv_dir, feature_dim, seq_len, median, num_samples, batch_size, is_middle, masked_idx=-1):
    model.eval()
...
def transformer_packet_detect(model, criterion, normal_dir, flow_dir, feature_dim, seq_len, median, num_samples, batch_size, is_middle, masked_idx=-1):
    # 已经得到了两个损失序列
    # 遍历 p 从 1 到 100，计算对应阈值下的 AUC
    ...
    if isinstance(normal_dir, list):
        ...
        gas_normal_list = scores_calculate('Transformer_packets_feature_model', 'reference', model, criterion, normal_dir, feature_dim, seq_len, median, num_samples, batch_size, is_middle, masked_idx)

    if isinstance(flow_dir, list):
        gas_nctc_list = flow_dir
    else:
        gas_nctc_list = scores_calculate('Transformer_packets_feature_model', 'detect', model, criterion, flow_dir, feature_dim, seq_len, median, num_samples, batch_size, is_middle, masked_idx)
    FPR_list = []
    TPR_list = []
...
def gas_transformer_detect(args):
    ...
    for nctc_dir_ in nctc_dirs:
        if nctc_dir_:
            ...
            for sub in (os.listdir(nctc_dir)):
