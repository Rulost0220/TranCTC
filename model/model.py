...
class MultiFieldCrossAttention(nn.Module):
    def __init__(
        ...
        if self.agg == "attn":
            ...
            self.pool_token = nn.Parameter(torch.randn(1, 1, d_feat))  #
    ...
    def feature_norm(self, x):
        ...
        if F == 7:
            ...
            if masked_idx != -1:  #
                ...
                #
                feature_max = feature_max[keep_indices]  #
...
class SeqClsDataset(Dataset):
    def __init__(self, X_np: np.ndarray, y_np: np.ndarray):
            ...
            self.X = torch.as_tensor(X_np, dtype=torch.long)  #
...
class IntervalDataset_Middle(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mid_idx = int((sample.shape[0] - 1) // 2)  #
...
def train_val(model, train_loader, val_loader, criterion, optimizer, epochs, save_dir, device):
    ...
    #
    val_bit_accuracies = [[] for _ in range(4)]  #
    val_bit_f1_scores = [[] for _ in range(4)]  #
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for batch_x, batch_y in val_loader:
                ...
                fv_label = (batch_y >> 3) & 1  #
                bv_label = (batch_y >> 2) & 1  #
                fs_label = (batch_y >> 1) & 1  #
                bs_label = batch_y & 1
